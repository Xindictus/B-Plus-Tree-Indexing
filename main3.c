...
int main() {
	 ...
	 ********************************************************************************/
	if ((scan1 = AM_OpenIndexScan(dAentry, EQUAL, (void *) edname)) < 0) {
		sprintf(errStr, "Error in AM_OpenIndexScan called on %s \n", deptName);
		AM_PrintError(errStr);
	}
	 ...
	 ********************************************************************************/
	while ((ivalue = (int*) AM_FindNextEntry(scan1)) != NULL) {
		printf("Age: %d \n", *ivalue);

		/*************************************************************************
		 *  Άνοιγμα της εμφωλιασμένης αναζήτησης                                 *
		 *************************************************************************/
		if ((scan2 = AM_OpenIndexScan(eAentry, EQUAL, (void *) ivalue)) < 0) {
			sprintf(errStr, "Error in AM_OpenIndexScan called on %s \n",
					empAge);
			AM_PrintError(errStr);
		}
		 ...
		 *************************************************************************/
		printf("Emp Names:\n ");
		while ((cvalue = (char*)AM_FindNextEntry(scan2)) != NULL) {
			printf("\t%s\n ", cvalue);
		}
		printf("\n");
